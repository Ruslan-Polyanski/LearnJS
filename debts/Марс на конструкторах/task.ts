...
function Spacecraft(name: string, fuelType: string): void {
  ...
  this.flight = function(distance: number) { // летим на нужное расстояние в нужную точну
    if(distance < 40) {
...
function MarsRover(model: string, navigation: {x: number, y: number}): void {
...
function MissionControl(missionName: string, launchDate: string): void {
      ...
      this.deployMarsRover = function() { // распологает марсоход на поверхности марса
        this.marsRover.isInTerrene = true;
      };
      this.coorginateMission = function(dataType: string) { // координирует миссию включая исследование марсохода
        if(dataType === 'stone') {
